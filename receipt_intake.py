"""
証憑取込（DW証憑Dropbox @346eqsyh）: LINE画像/ファイル → PDF → Dropbox _inbox/

2026-08-31 に dw-line-receipt-bot-v2（無料枠の月末枯渇問題）から、
常時稼働のこのサービス（Starter・支払済み）へ同居移設。追加費用ゼロ。
卵発注とはLINEチャネルも環境変数も別（RECEIPT_ プレフィックス）。
環境変数が無ければ configured()=False となり、main.py はマウントしない。
"""
import datetime
import io
import os
...
CHANNEL_SECRET = os.environ.get("RECEIPT_LINE_CHANNEL_SECRET", "")
CHANNEL_ACCESS_TOKEN = os.environ.get("RECEIPT_LINE_CHANNEL_ACCESS_TOKEN", "")
...
def configured() -> bool:
    return bool(CHANNEL_SECRET and CHANNEL_ACCESS_TOKEN and DROPBOX_REFRESH_TOKEN)
...
MAX_DIMENSION = 2200  # 長辺の最大ピクセル。512MB インスタンスで Tesseract を完走させる
...
def _image_to_pdf_pillow(normalized_jpeg: bytes) -> bytes:
    """Tesseract 不在環境用フォールバック（画像のみPDF）。"""
...
def image_to_pdf_with_text_layer(image_bytes: bytes) -> bytes:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 健全性検証: 完全な PDF は末尾近くに %%EOF を含む（6/20白紙事件の再発防止）
...
# ── LINE Webhook 再送による二重保存防止（メッセージIDマーカー） ──
...
def already_seen(message_id: str) -> bool:
    try:
        ...
        return True
    except Exception:  # noqa: BLE001  未検出は「未処理」扱い（安全側）
        return False
...
@router.get("/healthz")
def healthz():
    return {"status": "ok", "service": "receipt-intake (bundled)"}
...
async def webhook(request: Request):
    ...
    except Exception as e:  # noqa: BLE001  診断用に記録して再送出
...
def _content_to_pdf(content: bytes) -> bytes:
    """PDFはそのまま、画像(HEIC含む・pillow-heifで対応)はPDF化。"""
    if content[:5] == b"%PDF-":
        return content
    return image_to_pdf_with_text_layer(content)


def _save_pdf_and_reply(event, message_id: str, pdf_bytes: bytes) -> None:
...
if configured():
    ...
    def handle_file(event: MessageEvent):
        """写真を『ファイル』として送っても保存できる（PDF/HEIC/JPEG等）。"""
        _DIAG["image_handler_calls"] += 1
        message_id = event.message.id
        if already_seen(message_id):
            return
